...
import './ModalChangeVisit.scss';

interface IVisitFormDirector {
  performer: ISelectOption;
}

export const ModalChangeVisit = () => {
  const isShowModal = useModalsStore((state) => state.isChangeVisit);
  const changeShowModal = useModalsStore((state) => state.handleChangeVisit);
  ...
  const updateVisitPerformer = useVisitsStore(
