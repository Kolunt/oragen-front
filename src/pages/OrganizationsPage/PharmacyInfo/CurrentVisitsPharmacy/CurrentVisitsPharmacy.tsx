...
export const CurrentVisitsPharmacy = () => {
  const getVisits = useCurrentVisitsPharmacyStore((state) => state.getVisits);
  const currentPage = useCurrentVisitsPharmacyStore(
    (state) => state.currentPage
  );
  const pageSize = useCurrentVisitsPharmacyStore((state) => state.pageSize);
  const setApprovalStatus = useCurrentVisitsPharmacyStore(
    (state) => state.setApprovalStatus
  );
  const approvalStatus = useCurrentVisitsPharmacyStore(
    (state) => state.approvalStatus
  );
  const status = useCurrentVisitsPharmacyStore((state) => state.status);
  const setStatus = useCurrentVisitsPharmacyStore((state) => state.setStatus);
  ...
  return (
    <div className='CurrentsVisitsPharmacy flex-container hidden'>
