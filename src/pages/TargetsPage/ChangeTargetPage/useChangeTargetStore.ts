...
interface IChangeTargetStore {
  ...
  updateTarget: (id: number, payload: ICreateTargetPayload) => void;
}

export const useChangeTargetStore = create<IChangeTargetStore>()(
  immer((set, get) => ({
    drugs: [],
    performers: [],
    bricks: [],
    ...
    updateTarget: async (id, payload) => {
      ...
      try {
        const { data } = await updateTargetList(id, newTargetPayload);
        if (data.success) {
          useMessageStore
            .getState()
            .showMessage('success', 'Таргет-лист изменен!');
        }
      } finally {
        useAuthStore.getState().setLoading(false);
      }
